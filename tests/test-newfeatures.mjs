...
t('classify deposit ext+', ()=>{ const c=ctx.classifyLedgerDelta({type:'deposit',usdc:'1000'}); eq(c.ext,true); near(c.usdc,1000); });
t('classify withdraw ext-', ()=>{ const c=ctx.classifyLedgerDelta({type:'withdraw',usdc:'400'}); near(c.usdc,-400); });
t('classify vaultDeposit internal', ()=>{ eq(ctx.classifyLedgerDelta({type:'vaultDeposit',usdc:'9'}).ext,false); });
...
t('cashFlowModel core', ()=>{ const m=ctx.cashFlowModel(
  [{time:now-100*DAY,delta:{type:'deposit',usdc:'1000'}},{time:now-50*DAY,delta:{type:'withdraw',usdc:'200'}},{time:now-40*DAY,delta:{type:'vaultDeposit',usdc:'300'}}],1500,700);
  near(m.netExternal,800); eq(m.extCount,2); near(m.simpleReturn,700/800); ok(m.xirr!=null); });
